...
try_read_file(Path) ->
	NewPath = check_filepath(Path),
    case file:open(NewPath, [read]) of
	{ok, Fd} ->
	    file:close(Fd),
	    iolist_to_binary(NewPath);
	{error, Why} ->
	    ?ERROR_MSG("Failed to read ~s: ~s", [NewPath, file:format_error(Why)]),
	    erlang:error(badarg)
    end.


check_filepath(Path) when is_binary(Path) -> check_filepath(binary_to_list(Path));
check_filepath(Path) ->
     case filelib:is_file(Path) of
		 true -> list_to_binary(Path);
		 _ ->
			{ok,CurrentDirectory} = file:get_cwd(), 
			NewPath = lists:flatten([replace(CurrentDirectory,"/var/lib/ejabberd",""),Path]), 
	        case filelib:is_file(NewPath) of
		   		 true -> list_to_binary(NewPath);
...
read_file(Path) ->
	NewPath = check_filepath(Path),
    case file:read_file(NewPath) of
	{ok, Data} ->
	    {ok, Data};
	{error, Why} = Err ->
