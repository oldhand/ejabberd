...
add_new_user(From, Nick, Packet, State_Data) ->
    Lang = xmpp:get_lang(Packet),
    MaxUsers = get_max_users(State_Data),
    MaxAdminUsers = MaxUsers +
		      get_max_users_admin_threshold(State_Data),
    NUsers = dict:fold(fun (_, _, Acc) -> Acc + 1 end, 0,
		       State_Data#state.users),
			   
    StateData = case get_affiliation(From, State_Data) of
		none ->
		    NSD = set_affiliation(From, member, State_Data),
		    send_affiliation(From, member, State_Data),
		    store_room(NSD),
			NSD;
		_ ->
		    State_Data
...
can_change_ra(FAffiliation, _FRole, outcast, _TRole,
    ...
    when (FAffiliation == owner) or
	   (FAffiliation == admin) ->
    true;
can_change_ra(FAffiliation, _FRole, _, _TRole, affiliation, member, _ServiceAf)	-> true;
...
can_change_ra(FAffiliation, _FRole, member, _TRole,
	      affiliation, none, _ServiceAf)
    when (FAffiliation == owner) or (FAffiliation == member) or
	   (FAffiliation == admin) ->
    true;
...
send_wrapped_multiple(From, Users, Packet, Node, State) ->
	Muc = From#jid.user, 
	Affiliations = lists:map(fun({{Profileid,_,_},_}) -> Profileid end, ?DICT:to_list(State#state.affiliations)),
	OnlineUers = lists:map(fun({_, #user{jid = To}}) -> To#jid.user end, ?DICT:to_list(Users)), 
	spawn(fun()-> mod_offline_push:mucpush(Muc, OnlineUers,Affiliations,Packet) end),
    lists:foreach(
      fun({_, #user{jid = To}}) ->
	      send_wrapped(From, To, Packet, Node, State)
