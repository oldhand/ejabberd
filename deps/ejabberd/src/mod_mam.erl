...
muc_filter_message(#message{from = From} = Pkt,
    ...
    if Config#config.mam ->
	    ...
	    Affiliations = lists:map(fun({{Profileid,_,_},_}) -> Profileid end, ?DICT:to_list(MUCState#state.affiliations)),
	    case store_muc(MUCState, StorePkt, RoomJID, From, FromNick, Affiliations) of
		ok ->
...
-spec store_muc(mod_muc_room:state(), message(), jid(), jid(), binary() ,list())
      -> ok | pass | any().
store_muc(MUCState, Pkt, RoomJID, Peer, Nick,Affiliations) -> 
	...
	true ->
		...
		Pkt1 ->
		    ...
		    El = xmpp:encode(Pkt1),
		    Mod = gen_mod:db_mod(LServer, ?MODULE),
		    Mod:store(El, LServer, US, groupchat, Peer, Nick, recv, ID, Affiliations)
	    end;
	false ->
	    pass
    end.
